...
public class GetVisits extends AsyncTask<String, String, String> {

    private final String TAG = "GetVisits()";
    HttpURLConnection urlConnection;
    private Context mContext;
    private ProgressDialog pd;

    public GetVisits(Context context) {
        mContext = context;
    }
    ...
    protected String doInBackground(String... args) {
        ...
        URL url = null;
        try {
            url = new URL(AppMain._HOST_URL + VisitContract.singleFollowUps._URI);
    ...
    protected void onPostExecute(String result) {
        ...
        if (result != null) {
            ...
            if (json.length() > 0) {
                ArrayList<VisitContract> VisitsArrayList;
                DatabaseHelper db = new DatabaseHelper(mContext);
                try {
                    VisitsArrayList = new ArrayList<VisitContract>();
                    //JSONObject jsonObject = new JSONObject(json);
                    JSONArray jsonArray = new JSONArray(json);
                    db.syncVisits(jsonArray);
